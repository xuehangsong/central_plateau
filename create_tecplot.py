...
def read_xy_plane(input_file):
    """
    functions to read x,y plane data and reshape
    """
...
if case_name == "P2Rv8.3":
    case_dir = "/pic/projects/dvz/Hanford_ACMs/" + \
        "Intera_PlateauToRiver_CP-57037-Rev2/calib_final/"
    transport_dir = "/pic/projects/dvz/Hanford_ACMs/" + \
        "Intera_PlateauToRiver_CP-57037-Rev2/transport_files/prop/"
    nz = 7
    ...
    sy_refs = [case_dir+"sy" +
               str(iz+1)+".ref" for iz in range(nz)][::-1]  # 7 to 1
    bd_refs = [transport_dir+"bd" +
               str(iz+1)+".ref" for iz in range(nz)][::-1]  # 7 to 1
    ep_refs = [transport_dir+"ep" +
               str(iz+1)+".ref" for iz in range(nz)][::-1]  # 7 to 1
    sy_map = dict()
    kz_scale = 0.1
...
bd_data = read_multi_refs(bd_refs)
ep_data = read_multi_refs(ep_refs)
...
for iz in range(nz):
    ...
    bd_data[:, :, iz][srf_data[:, :, iz] == srf_data[:, :, iz+1]] = -1
    ep_data[:, :, iz][srf_data[:, :, iz] == srf_data[:, :, iz+1]] = -1
...
np.savetxt(output_dir+"model_grid_centroids.txt",
           ...
           fmt='%.8e',
           delimiter=',',
           header='Easting[m], Northing[m], Center Elevation [m, NAVD88],Bottom Elevation [m, NAVD88],Top Elevation [m, NAVD88], Ksxx[m/d], Ksyy[m/d], Kszz[m/d],Sy, Porosity,BulkDen')

# below is for tecplot
# prepare data for tecplot
varis = ["kz", "kh", "sy", "ep", "bd"]
varis_data = dict()
varis_data["kz"] = kz_data
varis_data["kh"] = hy_data
varis_data["sy"] = sy_data
varis_data["ep"] = ep_data
varis_data["bd"] = bd_data
